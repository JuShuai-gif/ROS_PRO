...
nav_msgs::Path path;
pathplanning::DijkstraConfig config;
...
void MapCallback(const nav_msgs::OccupancyGrid& msg)
{
    ...
    dijkstra.InitDijkstra(Map, Mask, config);
...
{
    //  Initial node
    ros::init(argc, argv, "diagonal");
    ros::NodeHandle nh;
    ros::NodeHandle nh_priv("~");
    ROS_INFO("Start diagonal node!\n");
    ...
    // Parameter
    nh_priv.param<bool>("Diagonal", config.Diagonal, true);
    ...
    // Advertise topics
    path_pub = nh.advertise<nav_msgs::Path>("dijkstra_path", 10);
    ...
    while(ros::ok())
    {
        if(start_flag)
        {
            ...
            // 记录dijkstra花费时间
            double Dijkstra_start_time = ros::Time::now().toSec();
            dijkstra.PathPlanning(startPoint, targetPoint, PathList);
            double Dijkstra_end_time = ros::Time::now().toSec();
            ROS_INFO("Dijkstra Use %f s", Dijkstra_end_time - Dijkstra_start_time);
